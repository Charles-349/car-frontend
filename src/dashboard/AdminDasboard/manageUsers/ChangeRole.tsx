...
const schema = yup.object({
    role: yup.string().oneOf(["user", "admin", "both"]).required("Role is required"),
});
...
const ChangeRole = ({ user }: ChangeRoleProps) => {
    ...
    const {
        ...
        defaultValues: {
            role: user ? (user.role as "user" | "admin" | "both") : "user", 
        },
    });
    ...
    useEffect(() => {
        if (user) {
            setValue("role", user.role as "user" | "admin" | "both"); 
        } else {
            reset();
        }
    }, [user, setValue, reset]);
    ...
    const onSubmit: SubmitHandler<ChangeRoleInputs> = async (data) => {
        try {
            ...
            await updateUser({ customerID: user.customerID, role: data.role })
